...
"""Recebe PIL.Image (RGB), retorna bytes JPEG (ou PNG se quiser).
iso: controla intensidade do Poisson (maior iso -> mais ruído visível)
...
prnu = generate_prnu_map(h, w, strength=prnu_strength, seed=seed)
lin_prnu = lin * prnu[..., None]


# escala relacionada ao ISO -- ajuste empirico
scale = max(1.0, 1024.0 / max(1, iso))
lam = np.clip(lin_prnu * 255.0 * scale, 0, None)


rng = np.random.default_rng(seed)
noisy_poisson = rng.poisson(lam).astype(np.float32) / (255.0 * scale)
...
noisy_lin = noisy_poisson[..., None] + gaussian
